...
static lv_res_t _create_mbox_autorcm_status(lv_obj_t *btn)
{
	...
	if (enabled)
	{
		lv_mbox_set_text(mbox,
			"オートRCMが#C7EA46 有効#になりました。\n\n"
			"#FF8000 電源ボタン#を押すだけで自動的にRCMに入ることができるようになりました。\n"
			"後で解除したい場合は、ここのボタンをもう一度押してください。");
	}
	else
	{
		lv_mbox_set_text(mbox,
			"オートRCMが#FF8000 無効#になりました。\n\n"
			"起動プロセスは正常になり、RCMに入るには#FF8000 音量+ボタン#と#FF8000 RCMジグ#の組み合わせが必要です。\n");
	}
...
static lv_res_t _create_mbox_hid(usb_ctxt_t *usbs)
{
	...
	static const char *mbox_btn_map[] = { "\251", "\262閉じる", "\251", "" };
	static const char *mbox_btn_map2[] = { "\251", "\222閉じる", "\251", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);

	char *txt_buf = malloc(SZ_4K);

	s_printf(txt_buf, "#FF8000 HIDエミュレーション#\n\n#C7EA46 デバイス：# ");

	if (usbs->type == USB_HID_GAMEPAD)
		strcat(txt_buf, "ゲームパッド");
	else
		strcat(txt_buf, "タッチパッド");
	...
	lv_label_set_static_text(lbl_tip, "注釈：終了するには#C7EA46 L3ボタン#と#C7EA46 ホームボタン#を押すか、ケーブルを外してください。");
	...
	if (!usbs->ro)
	{
		if (usbs->type == MMC_SD)
		{
			lv_label_set_static_text(lbl_tip,
				"注釈：終了するにはOS上で#C7EA46 マウント解除#を行ってください。\n"
				"            #FFDD00 絶対にケーブルを抜かないでください！#");
		}
		else
		{
			lv_label_set_static_text(lbl_tip,
				"注釈：終了するにはOS上で#C7EA46 マウント解除#を行ってください。\n"
				"            #FFDD00 マウントされていない場合は、ケーブルを取り外す必要がある場合があります。#");
		}
	}
	else
	{
		lv_label_set_static_text(lbl_tip,
			"注釈：終了するにはOS上で#C7EA46 マウント解除#を行うか\n            ケーブルを取り外してください。");
...
static lv_res_t _create_mbox_ums_error(int error)
{
	...
	{
	case 1:
		lv_mbox_set_text(mbox, "#FF8000 UMS#\n\n#FFFF00 SDカードのマウント中にエラーが発生しました。#");
		break;
	case 2:
		lv_mbox_set_text(mbox, "#FF8000 UMS#\n\n#FFFF00 アクティブなemuMMCが見つかりません。#");
		break;
	case 3:
		lv_mbox_set_text(mbox, "#FF8000 UMS#\n\n#FFFF00 アクティブなemuMMCはパーティションベースではありません。#");
		break;
	}
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_USB" USBツール");
	...
	lv_label_set_static_text(label_txt, "UMS");
	...
	lv_label_set_static_text(label_txt2,
		"SDカードをPCやスマホにマウントできます。\n"
		"#C7EA46 すべてのオペレーティング システムがサポートされています。アクセス権限は# #FF8000 読み取り/書き込み#です。");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"eMMC/emuMMCをマウントできます。\n"
		"#C7EA46 デフォルトのアクセス権限は# #FF8000 読み取り専用#です。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	nyx_create_onoff_button(lv_theme_get_current(), h_write,
		btn_write_access, SYMBOL_EDIT"読み取り専用   ", _emmc_read_only_toggle, false);
	...
	lv_label_set_static_text(label_txt3, "USB入力デバイス");
	...
	lv_label_set_static_text(label_btn, SYMBOL_CIRCUIT"  ゲームパッド");
	...
	lv_label_set_static_text(label_txt4,
		"ジョイコンを接続すると、\n"
		"デバイスがPCやスマホ用のゲームパッドに変わります。\n"
		"#C7EA46 両方のジョイコンが必要です。#");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_COPY" アーカイブビットを修正（全てのフォルダ）");
	...
	{
		lv_label_set_text(lb_desc, "#00DDFF SDカード内のすべてのファイルをスキャンします。#\nこれには少し時間がかかるかもしれません。");
		lv_obj_set_width(lb_desc, lv_obj_get_width(desc));
		...
		if (!total[0] && !total[1])
			s_printf(txt_buf, "#96FF00 完了！変更の必要はありませんでした。#");
		else
			s_printf(txt_buf, "#96FF00 完了！修正されたアーカイブビット：##FF8000 %d項目削除し、%d項目追加しました。#", total[1], total[0]);

		// Check errors.
		if (total[2] || total[3])
		{
			s_printf(txt_buf, "\n\n#FFDD00 エラー：フォルダアクセス：%d, アーカイブビットの修正：%d#\n"
					          "#FFDD00 ファイルシステムにエラーがないかチェックする必要があります。#",
					          total[2], total[3]);
		}
...
static lv_res_t _create_mbox_fix_touchscreen(lv_obj_t *btn)
{
	...
	lv_mbox_set_text(mbox,
		"#FFDD00 警告：問題がある場合にのみこれを実行してください。#\n\n"
		"#FF8000 電源ボタン#を押して続行\n#FF8000 音量ボタン#を押して中止");
	manual_system_maintenance(true);
	...
	else
	{
		...
		strcat(txt_buf, "#FFFF00 タッチスクリーンの調整に失敗しました。");
		lv_mbox_set_text(mbox, txt_buf);
		goto out2;
	}
...
out:
	if (res)
		lv_mbox_set_text(mbox, "#C7EA46 タッチスクリーンの調整が完了しました。");
	else
		lv_mbox_set_text(mbox, "#FFFF00 タッチスクリーンの調整に失敗しました。");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_MODULES" package1/2のダンプ");
	...
	if (!pkg1_id)
	{
		strcat(txt_buf, "#FFDD00 不明なpkg1バージョン#");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);

		if (!res)
		{
			strcat(txt_buf, "\n暗号化されたpkg1がpkg1_enc.binとしてダンプされました。");
			lv_label_set_text(lb_desc, txt_buf);
			manual_system_maintenance(true);
		}
	...
	if (h_cfg.t210b01 || kb <= HOS_KB_VERSION_600)
	{
		...
		{
			strcat(txt_buf, "#FFDD00 Pkg1の復号化に失敗しました。#\n");
			if (h_cfg.t210b01)
	...
	if (h_cfg.t210b01 || kb <= HOS_KB_VERSION_620)
	{
		...
		s_printf(txt_buf + strlen(txt_buf),
			"#C7EA46 NXブートローダーのサイズ：#0x%05X\n"
			"#C7EA46 セキュアモニターのアドレス：#0x%05X\n"
			"#C7EA46 セキュアモニターのサイズ：#0x%05X\n"
			"#C7EA46 ウォームブートのアドレス：      #0x%05X\n"
			"#C7EA46 ウォームブートのサイズ：       #0x%05X\n\n",
		...
		if (sd_save_to_file(pkg1, SZ_256K, path))
			goto out_free;
		strcat(txt_buf, "pkg1がpkg1_decr.binとしてダンプされました。\n");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);
		...
		if (sd_save_to_file(loader, hdr_pk11->ldr_size, path))
			goto out_free;
		strcat(txt_buf, "NXブートローダーnxloader.binとしてダンプされました。\n");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);
		...
		if (sd_save_to_file(secmon, hdr_pk11->sm_size, path))
			goto out_free;
		strcat(txt_buf, "セキュアモニターがsecmon.binとしてダンプされました。\n");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);
		...
		strcat(txt_buf, "Warmbootがwarmboot.binとしてダンプされました。\n\n");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);
	}
	...
	if (!pkg2_hdr)
	{
		strcat(txt_buf, "#FFDD00 Pkg2の複合化に失敗しました。#");
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);

		if (!res)
		{
			strcat(txt_buf, "\n暗号化されたpkg2がpkg2_encr.binとしてダンプされました。\n");
			lv_label_set_text(lb_desc, txt_buf);
			manual_system_maintenance(true);
		}
	...
	// Display info.
	s_printf(txt_buf + strlen(txt_buf),
		"#C7EA46 カーネルのサイズ：  #0x%05X\n"
		"#C7EA46 INI1のサイズ：    #0x%05X\n\n",
	...
	if (sd_save_to_file(pkg2, pkg2_hdr->sec_size[PKG2_SEC_KERNEL] + pkg2_hdr->sec_size[PKG2_SEC_INI1], path))
		goto out;
	strcat(txt_buf, "pkg2がpkg2_decr.binとしてダンプされました。\n");
	lv_label_set_text(lb_desc, txt_buf);
	manual_system_maintenance(true);
	...
	if (sd_save_to_file(pkg2_hdr->data, pkg2_hdr->sec_size[PKG2_SEC_KERNEL], path))
		goto out;
	strcat(txt_buf, "Kernelがkernel.binとしてダンプされました。\n");
	lv_label_set_text(lb_desc, txt_buf);
	manual_system_maintenance(true);
	...
	if (!ini1_off)
	{
		strcat(txt_buf, "#FFDD00 INI1とkipsのダンプに失敗しました。#\n");
		goto out;
	}
	...
	strcat(txt_buf, "INI1がini1.binとしてダンプされました。\n\n");
	lv_label_set_text(lb_desc, txt_buf);
	manual_system_maintenance(true);
	...
	for (u32 i = 0; i < ini1->num_procs; i++)
	{
		...
		s_printf(txt_buf + strlen(txt_buf), "%skipが%s.kip1としてダンプされました。\n", kip1->name, kip1->name);
		lv_label_set_text(lb_desc, txt_buf);
		manual_system_maintenance(true);
...
static void _create_tab_tools_emmc_pkg12(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_obj_t *label_txt = lv_label_create(h1, NULL);
	lv_label_set_static_text(label_txt, "バックアップと復元");
	...
	lv_label_set_static_text(label_btn, SYMBOL_UPLOAD"  eMMCのバックアップ");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"eMMCパーティションを個別に、または RAW イメージ全体として \n"
		"SDカードにバックアップできます。\n"
		"#FF8000 4GB##C7EA46 以上のSDカードをサポートします。#"
		"#C7EA46 （##FF8000 FAT32##C7EA46 と##FF8000 exFAT##C7EA46 ）#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  eMMCの復元");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"SDカードからeMMC/emuMMCパーティションを個別に、\n"
		"またはRAWイメージ全体として復元できます。\n"
		"#FF8000 4GB##C7EA46 以上のSDカードをサポートします。#"
		"#C7EA46 （##FF8000 FAT32##C7EA46 と##FF8000 exFAT##C7EA46 ）#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "SDカードのパーティションとUSB");
	...
	label_btn = lv_label_create(btn3, NULL);
	lv_btn_set_fit(btn3, true, true);
	lv_label_set_static_text(label_btn, SYMBOL_SD"  SDカードのパーティション");
	lv_obj_align(btn3, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"SDカードの#C7EA46 emuMMC#、#C7EA46 Android#、#C7EA46 Linux#で使用するための\nパーティションを作成できます。\n"
		"LinuxやAndroidをフラッシュすることもできます。\n");
	lv_obj_set_style(label_txt4, &hint_small_style);
	lv_obj_align(label_txt4, btn3, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 3);
	...
	label_btn = lv_label_create(btn4, NULL);
	lv_label_set_static_text(label_btn, SYMBOL_USB"  USBツール");
	lv_obj_align(btn4, label_txt4, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 2);
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"#C7EA46 UMS#、#C7EA46 ゲームパッド#その他のUSBツールです。\n"
		"UMSはSD、eMMC、emuMMCをマウントできます。\n"
		"ゲームパッドはSwitchを入力デバイスに変換できます。#");
	lv_obj_set_style(label_txt4, &hint_small_style);
	lv_obj_align(label_txt4, btn4, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 3);
}
...
static void _create_tab_tools_arc_autorcm(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_obj_t *label_btn = lv_label_create(btn, NULL);
	lv_btn_set_fit(btn, true, true);
	lv_label_set_static_text(label_btn, SYMBOL_DIRECTORY"  アーカイブビットの修正");
	lv_obj_align(btn, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"ルートやemuMMCの『Nintendo』フォルダーを含む\n"
		"すべてのフォルダーのアーカイブビットを修正できます。\n"
	...
	lv_obj_t *btn2 = lv_btn_create(h1, btn);
	label_btn = lv_label_create(btn2, NULL);
	lv_label_set_static_text(label_btn, SYMBOL_KEYBOARD"  タッチスクリーンの調整");
	lv_obj_align(btn2, label_txt2, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 2);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"タッチスクリーンモジュールを調整できます。\n"
		"#FF8000 これにより、#\n#FF8000 NyxおよびHOSのタッチスクリーンに関する問題が修正されます。#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, btn2, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 3);
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "その他");
	lv_obj_set_style(label_txt3, th->label.prim);
	lv_obj_align(label_txt3, label_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, -LV_DPI * 3 / 10);
	...
	lv_label_set_text(label_btn, SYMBOL_REFRESH"  オートRCM #00FFC9   ON #");
	lv_obj_align(btn3, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	s_printf(txt_buf,
		"#C7EA46 音量ボタン+#と#C7EA46 RCMジグ#を使わずにRCMに入ることができます。\n"
		"#FF8000 要求があればいつでもオートRCMのすべてのバージョンを復元できます。#\n"
		"#FF3C28 これによりBCTが破損し、\nカスタムブートローダーなしでは起動できなくなります。#");

	if (h_cfg.rcm_patched)
		strcat(txt_buf, " #FF8000 このユニットにはパッチが適用されているため、これは無効になっています。#");
	...
	label_btn = lv_label_create(btn4, NULL);
	lv_label_set_static_text(label_btn, SYMBOL_MODULES"  Package1/2をダンプ");
	lv_obj_align(btn4, label_txt4, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 2);
...
void create_tab_tools(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_obj_t *tab1= lv_tabview_add_tab(tv, "eMMC "SYMBOL_DOT" SDカードのパーティション "SYMBOL_DOT" USB");
	lv_obj_t *tab2 = lv_tabview_add_tab(tv, "アーカイブビット "SYMBOL_DOT" RCM "SYMBOL_DOT" タッチ "SYMBOL_DOT" Pkg1/2");
