...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	{
		s_printf(gui->txt_buf, "\n#FFDD00 ファイルベースのemuMMCに必要な空き容量が足りません。#\n");
...
static int _dump_emummc_raw_part(emmc_tool_gui_t *gui, int active_part, int part_idx, u32 sd_part_off, emmc_part_t *part, u32 resized_count)
{
	...
	s_printf(gui->txt_buf, "#96FF00 ベースフォルダ：#\n%s\n#96FF00 パーティションオフセット：# #FF8000 0x%08X#",
	...
	while (totalSectors > 0)
	{
		...
		{
			s_printf(gui->txt_buf,
				"\n#FFDD00 SDカードから%dブロックの読み込み中にエラーが発生しました。@LBA %08X,#\n"
				"#FFDD00 (試行回数 %d) #",
				num, lba_curr, ++retryCount);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (resized_count)
	{
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, "完了！\n");
		...
		s_printf(gui->txt_buf, "USERパーティションをフォーマット中... \n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		if (mkfs_error)
		{
			s_printf(gui->txt_buf, "#FF0000 失敗しました。(%d)#\nもう一度試してください。\n", mkfs_error);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		...
		s_printf(gui->txt_buf, "新しいGPTパーティションを書き込み中... ");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
...
static int _emummc_raw_derive_bis_keys(emmc_tool_gui_t *gui, u32 resized_count)
{
	...
	if (memcmp(&cal0->magic, "CAL0", 4))
	{
		...
		strcpy(txt_buf, "#FFDD00 BISキーの検証に失敗しました。#\n");
		error = true;
	}
	...
	if (error)
	{
		...
		lv_mbox_set_text(mbox, "#C7EA46 BISキーの生成#");
...
void dump_emummc_raw(emmc_tool_gui_t *gui, int part_idx, u32 sector_start, u32 resized_count)
{
	...
	{
		s_printf(gui->txt_buf, "#FFDD00 USERパーティションをフォーマットするには#\n#FFDD00 BISキーが必要です。#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		emmc_end();
		goto out;
	}
