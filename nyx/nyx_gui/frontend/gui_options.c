...
disabled:;
	...
	if (!error)
	{
		s_printf(txt_buf,
			"SDカードへのダンプが完了しました。\n"
			"#C7EA46 switchroot/joycon_mac.[bin/ini]#に保存しました。\n\n");
		...
		{
			strcat(txt_buf, "#FF8000 警告：##FFDD00 左側#のペアリングデータはHOSベースではありません。");
			success = false;
		}
		else if (is_l_hos && !is_r_hos)
		{
			strcat(txt_buf, "#FF8000 警告：# #FFDD00 右側#のペアリングデータはHOSベースではありません。");
			success = false;
		}
		else
		{
			strcat(txt_buf, "#FF8000 警告：#HOSベースのペアリング データはありません。");
			success = false;
		}

		if (!success)
			strcat(txt_buf,
				"\n\n#FFDD00 両方のジョイコンが接続されていることと、#\n"
				"#FFDD00 HOSでペアリングされていることを確認してください。#");
	}
	else
	{
		if (!nx_hoag)
			...
			s_printf(txt_buf, "#FFDD00 Switch Liteではサポートされていません！#\n");
	}
