...
lv_obj_t *create_window_autoboot(const char *win_title)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" 閉じる", _win_autoboot_close_action);
...
{
	lv_obj_t *win = create_window_autoboot(SYMBOL_GPS" 自動起動");
	lv_win_add_btn(win, NULL, SYMBOL_POWER" 無効", _autoboot_disable_action);
	...
	lv_label_set_static_text(label_txt, "メイン設定");
	...
	lv_label_set_static_text(label_txt3, "iniフォルダ設定");
...
static lv_res_t _save_nyx_options_action(lv_obj_t *btn)
{
	...
	if (res)
		lv_mbox_set_text(mbox, "#FF8000 Nyx設定#\n\n#96FF00 設定をSDカードに保存しました。#");
	else
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_COPY" Nyxカラーテーマ");
	lv_win_add_btn(win, NULL, SYMBOL_HINT" 背景切り替え", _preset_bg_action);
	lv_win_add_btn(win, NULL, SYMBOL_SAVE" 保存して再読み込み", _save_theme_color_action);
	...
	lv_label_set_static_text(lbl_sample, "サンプル：");
	...
	lv_label_set_static_text(lbl_test,
		"患者が患者をケアすることは非常に重要であり、"
		"それが患者の成長につながりますが、同時に多くの労力と痛みが発生することもあります。"
		"というのは、最も細かい点に至ると、そこから何らかの利益を得ない限り、"
		"いかなる種類の仕事も実践すべきではないからである。");
...
static lv_res_t _create_mbox_clock_edit(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\251", "\222完了", "\222キャンセル", "\251", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "Nyxの#C7EA46 日付#と#C7EA46 時刻#を入力します。\nこの設定では。本体時刻は変更されません。");
...
disabled:;
	...
	if (!error)
	{
		if (!nx_hoag)
		{
			s_printf(txt_buf,
				"SDカードへのダンプが完了しました\n"
				"保存先：#C7EA46 switchroot/joycon_mac.[bin/ini]#\n\n");

			bool success = true;

			// Check if pairing info was found.
			if (joycon_found == 2)
				strcat(txt_buf, "#C7EA46 成功！#\n#C7EA46 ジョイコンのペアリングデータが2個中2個見つかりました！#\n");
			else
			{
				s_printf(txt_buf + strlen(txt_buf), "#FF8000 失敗！#\n#FF8000 警告：##FFDD00 %d/2#このペアリングデータが見つかりました。\n", joycon_found);
				success = false;
			}

			// Check if pairing was done in HOS.
			if (is_l_hos && is_r_hos)
				strcat(txt_buf, "#C7EA46 どちらのペアリングデータもHOSベースです。#");
			else if (!is_l_hos && is_r_hos)
			{
				strcat(txt_buf, "#FF8000 警告：##FFDD00 左#のペアリングデータはHOSベースではありません。");
				success = false;
			}
			else if (is_l_hos && !is_r_hos)
			{
				strcat(txt_buf, "#FF8000 警告：##FFDD00 右#のペアリングデータはHOSベースではありません。");
				success = false;
			}
			else
			{
				strcat(txt_buf, "#FF8000 警告：#HOSベースのペアリングデータがありません。");
				success = false;
			}

			if (!success)
				strcat(txt_buf,
					"\n\n#FFDD00 両方のジョイコンが接続されていることを確認し、#\n"
					"#FFDD00　HOSでペアリングしたことを確認してください。#");
		}
		else
		{
			s_printf(txt_buf,
				"SDカードへのダンプが完了しました\n"
				"保存先：#C7EA46 switchroot/lite_gamepad.cal#\n\n");
			strcat(txt_buf, "#C7EA46 成功！#\n#C7EA46 Liteゲームパッドデータが見つかりました。#\n");
		}
	}
	else
	{
		if (!nx_hoag)
			s_printf(txt_buf, "#FFDD00 ジョイコンのペアリング情報のダンプに失敗しました。#\n#FFDD00 エラー：%d#", error);
		else
			s_printf(txt_buf, "#FFDD00 Liteゲームパッド情報の取得に失敗しました。#\n");
	}
...
lv_res_t create_win_nyx_options(lv_obj_t *parrent_btn)
{
	...
	lv_obj_t *win = nyx_create_window_custom_close_btn(SYMBOL_HOME" Nyx設定", _action_win_nyx_options_close);
	...
	lv_label_set_static_text(label_btn, SYMBOL_COPY" カラーテーマ");
	...
	lv_label_set_static_text(label_txt2, "Nyxのすべての#00FFC8 ハイライト#の色を選択します。\n");
	...
	lv_label_set_static_text(label_txt, SYMBOL_HOME" ホーム画面");
	...
	lv_label_set_static_text(label_txt2,
		"Nyx起動時に表示する画面を選択します。\n"
		"#FF8000 設定：##C7EA46 追加の構成を『起動』の空のスロットに結合します。#\n"
		"#FF8000 起動 / その他:##C7EA46 古典的な分割ビューを使用します。#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	nyx_create_onoff_button(th, sw_h2, btn2, SYMBOL_GPS" 拡張起動エントリ", _entries_columns_action, true);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"1行あたりのブート エントリを 5 に設定します。(デフォルトは4)\n"
		"#C7EA46 これにより、合計10個のブート エントリが『起動』と『その他』に#\n"
		"#C7EA46 表示されるようになります。#\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_btn3, SYMBOL_DOWNLOAD" ジョイコンBTをダンプ");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"SwitchとジョイコンのMACアドレスと、\n"
		"それらに関連付けられたLTKを保存できます。(#C7EA46 Android#と#C7EA46 Linux#の場合)");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_txt, SYMBOL_MODULES_ALT" データ検証");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2, "バックアップと復元のために実行されるデータ検証のタイプを設定します。\n"
		"バックアップ/復元を失うことなくキャンセルできます。\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	lv_label_set_static_text(label_btn5, SYMBOL_CLOCK" 時計（オフセット）");
	...
	lv_label_set_static_text(label_txt2,
		"時計オフセットを手動で変更します。\n"
		"#C7EA46 入力された日付と時刻は自動的にオフセットに変換されます。#\n"
		"#C7EA46 これはFatFS操作にも使用されます。#");
	lv_obj_set_style(label_txt2, &hint_small_style);
...
void create_tab_options(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_btn_set_fit(btn, true, true);
	lv_btn_set_toggle(btn, true);
	lv_label_set_static_text(label_btn, SYMBOL_GPS" 自動起動 #00FFC9   ON #");
	...
	lv_obj_t *label_txt2 = lv_label_create(sw_h2, NULL);
	lv_label_set_static_text(label_txt2, "hekate起動時に自動起動する起動エントリまたはペイロードを選択します。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_obj_t *label_txt = lv_label_create(l_cont, NULL);
	lv_label_set_static_text(label_txt, SYMBOL_CLOCK" 起動時間の遅延  ");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"自動起動が有効な場合に起動ロゴを表示する時間を設定します。\n"
		"#C7EA46 その間に# #FF8000 音量-ボタン##C7EA46 を押すと、hekateのメニューに入ることができます。#\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	lv_obj_t *btn2 = lv_btn_create(sw_h2, NULL);
	nyx_create_onoff_button(th, sw_h2, btn2, SYMBOL_SHRK" 自動ヒューズ焼き回避", auto_nogc_toggle, true);
	lv_obj_align(btn2, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 10);
	...
	lv_label_set_static_text(label_txt2,
		"ヒューズをチェックし、\n"
		"ファームウェアが高い場合は自動的にパッチを適用します。\n"
		"これはグローバル設定となり、デフォルトで自動に設定されます。(ON: 自動)\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, btn2, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 12);
	...
	lv_obj_t *btn3 = lv_btn_create(sw_h3, NULL);
	nyx_create_onoff_button(th, sw_h3, btn3, SYMBOL_POWER" HOS自動電源オフ", auto_hos_poweroff_toggle, true);
	lv_obj_align(btn3, label_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);

	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2,
		"HOSからシャットダウンすると、デバイスは15秒後にウェイクアップします。\n"
		"有効にすると次回のhekate起動時に自動的に電源がオフになります。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	label_txt = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt, SYMBOL_BRIGHTNESS" 画面の明るさ");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2, "画面の明るさを設定します。\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	nyx_create_onoff_button(th, sw_h3, btn4, SYMBOL_REFRESH" リブートtoペイロードの更新", _update_r2p_action, true);
	lv_obj_align(btn4, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 10);

	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"選択した起動エントリでFSS0が使用されている場合、\nリブートtoペイロードバイナリがチェックされ、\n"
		"強制的にhekateに更新されます。\n\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
